...
import { formatMoney, formatPercent, statusLabel, tierClass, tierLabel } from "./lib/format.js";
...
function DraftAction({ action, invoiceNumber, queuedActions, onQueue }) {
  const actionKey = `${invoiceNumber}-${action.exception_code}-${action.action_type}`;
  const queued = queuedActions.has(actionKey);
  const isApprovalNote = action.action_type === "approval_note";
      ...
      <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
        ...
        {!isApprovalNote ? (
          <button
            ...
            type="button"
            disabled={queued}
            onClick={() => onQueue(actionKey)}
          >
            {queued ? "Queued" : "Approve & Queue"}
          </button>
        ) : null}
      </div>
...
function InvoiceCard({ match, classification, actionResult, queuedActions, onQueue }) {
  ...
  return (
    <article className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <h3 className="text-xl font-semibold">{match.invoice_number}</h3>
          <p className="mt-1 text-sm text-slate-600">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {exceptions.length ? (
          exceptions.map((code) => (
            <Badge className="border-red-200 bg-red-50 text-red-800" key={code}>{code}</Badge>
          ))
        ) : (
          <Badge className="border-green-200 bg-green-50 text-green-800">Clean</Badge>
        )}
      ...
      {financial ? (
        <div className="mt-4 grid gap-3 text-sm sm:grid-cols-3">
          <div className="rounded-md bg-slate-50 p-3">
            ...
            <p className="font-semibold">{formatMoney(financial.total_exposure)}</p>
          </div>
          <div className="rounded-md bg-slate-50 p-3">
            <p className="text-slate-500">Hold</p>
            <p className="font-semibold">{formatMoney(financial.total_hold)}</p>
          </div>
          <div className="rounded-md bg-slate-50 p-3">
            <p className="text-slate-500">Approved</p>
            <p className="font-semibold">{formatMoney(financial.total_approved)}</p>
          </div>
        </div>
      ) : null}

      <details className="mt-4 rounded-md border border-slate-200 p-3">
        <summary className="cursor-pointer text-sm font-semibold">Reasoning</summary>
        <p className="mt-3 text-sm leading-6 text-slate-600">{match.reasoning}</p>
        {classification?.tier_rationale ? (
          <p className="mt-3 text-sm leading-6 text-slate-600">{classification.tier_rationale}</p>
        ) : null}
      </details>

      {actionResult?.actions?.length ? (
...
export default function App() {
  ...
  const [queuedActions, setQueuedActions] = useState(new Set());
      ...
      <section className="mx-auto flex max-w-7xl flex-col gap-6">
        ...
        {matchResults?.results?.length ? (
          <section className="grid gap-4">
            {matchResults.results.map((match, index) => (
              <InvoiceCard
                ...
                queuedActions={queuedActions}
                onQueue={(actionKey) => {
                  setQueuedActions((current) => new Set([...current, actionKey]));
                }}
              />
            ))}
          </section>
